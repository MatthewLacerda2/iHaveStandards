...
**5. Finish** — when all tasks are done and `make check` is green, open a PR per
CLAUDE.md (Context / Solution / Surface / Result; assign the user; if it
implements an issue, title it `{issue}-{branch}` and start the body with
`Closes #{issue}`). **Never merge without the user's say-so.**
